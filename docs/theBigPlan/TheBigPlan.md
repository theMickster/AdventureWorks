...
- 🚧 #936 Work Orders / Manufacturing Module — In Progress (4 child features); ✅ US-966 Work Orders List API done (`GET /api/v1/work-orders` — paginated/filterable, computed `yieldRate` and `isCompletedLate`); ✅ Feature 962 US-967/968 done (`WorkOrderStore` + `WorkOrderListComponent` at `/manufacturing/work-orders`); ✅ Feature 963 US-969/970 done (`GET /api/v1/work-orders/{id}` with computed `daysLate` and `scrapReasonName`; `WorkOrderDetailComponent` at `/manufacturing/work-orders/:id`)
