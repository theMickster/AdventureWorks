...
- ✅ #935 Customer Module complete (2026-08-22) — Feature 947, US-951/952/953 (Customer LTV list, API + Angular); ✅ Feature 948, US-954/955/956 (Customer detail profile — LTV rank, metric tiles, inactive badge, store link); ✅ Feature 949, US-957/958/959 (customer-scoped sales-order list/analytics and customer-detail recent orders)
- ✅ Additional Initiative #559 backlog epics: #936 (Work Orders/Manufacturing) Complete — ✅ US-966 Work Orders List API (`GET /api/v1/work-orders`, `WorkOrderRepository.GetWorkOrdersAsync`/`SearchWorkOrdersAsync`, server-computed `yieldRate`/`isCompletedLate`); ✅ Feature 962, US-967/968 (Angular `WorkOrderStore` + `WorkOrderListComponent`, `/manufacturing/work-orders` — new `libs/manufacturing/data-access` and `libs/manufacturing/feature-work-orders`, `scope:manufacturing`); ✅ Feature 963, US-969/970 (Work Order Detail — `GET /api/v1/work-orders/{id}` with `daysLate`/`scrapReasonName`, `WorkOrderDetailComponent` at `/manufacturing/work-orders/:id`); ✅ Feature 964, US-971/972 (Manufacturing KPI dashboard — `GET /api/v1/manufacturing/kpis`, aggregate yield/scrap metrics, `ManufacturingKpiDashboardComponent` at `/manufacturing`); ✅ Feature 965, US-973/974/975 (Manufacturing Quality Scorecard — `GET /api/v1/manufacturing/quality-scorecard`, top scrap and bottom yield product rankings, scrap-reason breakdown, and dashboard filter drill-down); #937 (Purchasing/Vendor) In Progress — ✅ Feature 977, US-981/982/983 (Vendor list ranked by spend with credit risk indicators, API + Angular UI); ✅ Feature 978, US-984/985 (Vendor Detail API + profile UI with PO history — `GET /api/v1/vendors/{id}`, `VendorRepository.GetVendorDetailAsync`); ✅ Feature 979, US-986/987 (Purchase Order Detail API + Angular UI — `GET /api/v1/purchase-orders/{id}`, `PurchaseOrderRepository.GetPurchaseOrderDetailAsync`, null-safe approving-employee name via explicit LEFT JOIN, plus additive `VendorName`/`ShipMethodName`/`ProductName` server-side joins; `PurchaseOrderDetailComponent` in `libs/purchasing/feature-vendors` at `/purchasing/purchase-orders/:id` replaces the earlier placeholder); ✅ Feature 980, US-988/989/990 (Procurement Analytics — Pareto Chart + Pipeline Summary; `GET /api/v1/purchasing/analytics`, `PurchasingAnalyticsRepository` with C#-computed cumulative spend percent over all 104 vendors and a 4-status pipeline summary; new `libs/purchasing/ui-pareto-chart` Chart.js mixed bar/line component with inline 80%/95% reference-line plugin, `PurchasingAnalyticsComponent` at `/purchasing/analytics` — completes #937)
- 🚧 Epic #569 (Polyglot Azure Functions Architecture) started: ✅ US 806–810 Sales Order Saga (durable validation/inventory/reservation, payment authorization and external-result correlation, confirmation, exact-allocation compensation, and SQL retries) implemented; deployment verification remains.
...
- **ADO rollup**: Epics — Done 4, In Progress 1, New 4. Features — Done 36, In Progress 8, New 16.
...
#### Epic #935 -- Customer Module ✅ COMPLETE (2026-08-22)
...
- ✅ Feature #949, US-957/958/959 Customer-Scoped Orders — `GET /v1/sales-orders?customerId={id}` and `POST /v1/sales-orders/analytics` share validated `customerId` filtering through `SalesOrderRepository`; `CustomerDetailComponent` loads the five newest orders and exposes View Orders/View All/detail links; `OrderListComponent` persists `customerId` in its reactive URL, analytics payload, reset flow, and order-detail back navigation.
...
- 🚧 #562 HR CRUD UI — In Progress (5 child features; Departments CRUD list/detail/create/edit implemented; ✅ US-756 Create Employee wizard implemented; ✅ US-757 Edit Employee Personal Info inline implemented; ✅ Feature 719 (Interactive Organization Chart) done — US-762–764 complete, `hr/feature-org-chart` at `/hr/org-chart`, recursive `OrgNodeComponent`, `OrgChartStore`, department-group color-coding, search/click-through, summary stats; ✅ Feature 721 (Employee History and Payroll UI) done — US-768/769/770 complete, new hr/feature-history lib (DepartmentTimelineComponent, PayHistoryTableComponent, CombinedTimelineComponent; client-side merge of existing department-history/pay-history endpoints, no new backend work) wired into EmployeeDetailComponent's tab bar)
- 📋 #563 Real-Time Dashboard — New (5 child features)
- ✅ #936 Work Orders / Manufacturing Module — Complete (5 child features); ✅ US-966 Work Orders List API done (`GET /api/v1/work-orders` — paginated/filterable, computed `yieldRate` and `isCompletedLate`); ✅ Feature 962 US-967/968 done (`WorkOrderStore` + `WorkOrderListComponent` at `/manufacturing/work-orders`); ✅ Feature 963 US-969/970 done (`GET /api/v1/work-orders/{id}` with computed `daysLate` and `scrapReasonName`; `WorkOrderDetailComponent` at `/manufacturing/work-orders/:id`); ✅ Feature 964 US-971/972 done (`GET /api/v1/manufacturing/kpis` aggregate manufacturing metrics and `ManufacturingKpiDashboardComponent` at `/manufacturing`); ✅ Feature 965 US-973/974/975 done (`GET /api/v1/manufacturing/quality-scorecard` with top scrap/bottom yield rankings and scrap-reason breakdown; dashboard tables link to existing `productId`/`scrapReasonId` work-order filters)
- ✅ #937 Purchasing / Vendor Management Module — Complete (4 child features); ✅ Feature 977 US-981/982/983 done — `GET /api/v1/vendors` (`VendorRepository.GetVendorsAsync`), paginated vendor list ranked by total spend descending with pre-computed `isHighRisk` flags (CreditRating >= 4 and pre-filter spend-rank <= 52); `VendorStore` (`libs/purchasing/data-access`) + `VendorListComponent` (`libs/purchasing/feature-vendors`) at `/purchasing/vendors` complete the Angular UI; ✅ Feature 978 US-984/985 done — `GET /api/v1/vendors/{id}` (`VendorRepository.GetVendorDetailAsync`, spend metrics + PO history), vendor profile UI; ✅ Feature 979 US-986/987 done — `GET /api/v1/purchase-orders/{id}` (`PurchaseOrderRepository.GetPurchaseOrderDetailAsync`), full purchase order detail with line items and a null-safe approving-employee name (falls back to "Unknown employee" via an explicit LEFT JOIN rather than `Include`, since AdventureWorks data can contain unresolvable `EmployeeId` references), plus additive `VendorName`/`ShipMethodName`/`ProductName` server-side joins (plain `.Include()`, no null-safety edge case); `PurchaseOrderDetailComponent` (`libs/purchasing/feature-vendors`) at `/purchasing/purchase-orders/:id` completes the Angular UI, replacing the earlier `PurchaseOrderDetailPlaceholderComponent`; ✅ Feature 980 US-988/989/990 done — `GET /api/v1/purchasing/analytics` (`PurchasingAnalyticsRepository`), Pareto vendor-spend data (all 104 vendors, C#-computed running cumulative percent forced to exactly 100 on the last row) plus a 4-status PO pipeline summary (Pending/Approved/Rejected/Complete, zero-backfilled); new `libs/purchasing/ui-pareto-chart` (`ParetoChartComponent`, Chart.js mixed bar/line chart with an inline `afterDatasetsDraw` plugin drawing 80%/95% reference lines — no `chartjs-plugin-annotation` dependency added) and `PurchasingAnalyticsComponent` at `/purchasing/analytics` complete the Angular UI
...
- ✅ US 808-810 implemented: outbound configurable payment authorization, Service Bus payment results with a durable 30-minute timeout, confirmation/approval events, exact inventory compensation, and transient SQL retries.
