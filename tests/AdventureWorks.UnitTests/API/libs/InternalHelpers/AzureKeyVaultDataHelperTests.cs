...
public sealed class AzureKeyVaultDataHelperTests : UnitTestBase
{
    ...
    [Fact]
    public void GetAzureKeyVaultClient_when_tenant_id_null_throws_correct_exception()
    {
        var settings = new Dictionary<string, string>
        {
            {"Key1", "Value1"},
            {"KeyVault:VaultUri", "https://akv.com"}
        };
        ...
        _ = ((Action)(() => _ = AzureKeyVaultDataHelper.GetAzureKeyVaultClient(configuration)))
            .Should().Throw<ConfigurationException>("because we expect a custom configuration exception.")
            .And.Message.Should().Contain(ConfigurationConstants.KeyVaultTenantId);
    }


    [Fact]
    public void GetAzureKeyVaultClient_when_client_id_null_throws_correct_exception()
    {
        var settings = new Dictionary<string, string>
        {
            { "Key1", "Value1" },
        ...
        _ = ((Action)(() => _ = AzureKeyVaultDataHelper.GetAzureKeyVaultClient(configuration)))
            .Should().Throw<ConfigurationException>("because we expect a custom configuration exception.")
            .And.Message.Should().Contain(ConfigurationConstants.KeyVaultClientId);
    }

    [Fact]
    public void GetAzureKeyVaultClient_when_client_secret_null_throws_correct_exception()
    {
        var settings = new Dictionary<string, string>
        {
            { "Key1", "Value1" },
        ...
        _ = ((Action)(() => _ = AzureKeyVaultDataHelper.GetAzureKeyVaultClient(configuration)))
            .Should().Throw<ConfigurationException>("because we expect a custom configuration exception.")
            .And.Message.Should().Contain(ConfigurationConstants.KeyVaultClientSecret);
    }
}