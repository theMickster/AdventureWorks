...
public sealed class ReserveStockActivityCore(SalesOrderSagaDbContext dbContext) : TaskActivity<SalesOrderSagaInput, ReservationReceipt>
{
    public override async Task<ReservationReceipt> RunAsync(TaskActivityContext context, SalesOrderSagaInput? input)
    {
        ...
        var sagaInstanceId = SagaIds.InstanceIdFor(input.SalesOrderId);
        var existingAllocations = await dbContext.InventoryAllocations
            .Where(a => a.SagaInstanceId == sagaInstanceId)
            .ToListAsync();

        if (existingAllocations.Count > 0)
        {
            // A historical AdventureWorks sales transaction can have the same order ID. Only
            // the saga-owned allocation rows prove this specific durable instance reserved stock.
            var reservedAt = existingAllocations.Min(a => a.ReservedAt);

            return new ReservationReceipt(
                input.SalesOrderId,
                existingAllocations.Select(a => a.ProductId).Distinct().ToList(),
        ...
        for (var lineIndex = 0; lineIndex < input.Lines.Count; lineIndex++)
        {
            ...
            var allocations = await DecrementInventoryAsync(line);
    ...
    /// </summary>
    private async Task<IReadOnlyList<(short LocationId, short Quantity)>> DecrementInventoryAsync(SalesOrderSagaLineItem line)
    {
        var remaining = (int)line.OrderQty;
        var allocations = new List<(short LocationId, short Quantity)>();
        ...
        foreach (var row in inventoryRows)
        {
            ...
            remaining -= decrement;
            allocations.Add((row.LocationId, decrement));
        }
