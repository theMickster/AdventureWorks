...
**VendorListComponent** — server-side paginated, filterable vendor list at `/purchasing/vendors`. Now has a detail route: each row's "View" button (mirroring `WorkOrderListComponent`'s View-button precedent) navigates to `/purchasing/vendors/:id`.
...
**VendorDetailComponent** (Feature 978, US-984/985) — vendor profile with spend metrics and filterable purchase order history at `/purchasing/vendors/:id`. Direct `PurchasingApiService` calls; no NgRx store, mirroring `OrderDetailComponent`/`StoreDetailComponent`.

- **Sequential fetch, not parallel**: `loadVendor()` only calls `startPurchaseOrderHistorySync()` — which subscribes to `route.queryParams` and issues the first PO-history request — inside the vendor-detail success callback. A 404'd or errored vendor never triggers a PO-history request at all. This was a deliberate design decision (caught in a `/cmon-bro` self-review pass on the original plan): firing both requests in parallel risks a wasted/erroring PO-history call racing the "Vendor not found" `EmptyState` for a vendor id that doesn't exist.
- **`startPurchaseOrderHistorySync` uses `switchMap`, not a raw per-emission `subscribe()`** (caught in a second `/cmon-bro` pass, after the feature was believed complete): a naive `route.queryParams.subscribe(params => api.getVendorPurchaseOrders(...).subscribe(...))` lets two in-flight requests race if the user changes the filter again before the first response lands — the response that resolves _last_ wins regardless of which query is actually current. `switchMap` cancels the stale inner request when a new `queryParams` emission arrives, mirroring `VendorStore.loadPage`'s `switchMap`-based `rxMethod` cancellation guard even though this component has no store. `vendor-detail.spec.ts` has a regression test that fails against the naive implementation (verified by hand) and passes with `switchMap`.
- **Two independent loading/error signal groups**: `isLoadingVendor`/`vendorNotFound`/`vendorHasError` vs. `isLoadingPurchaseOrders`/`purchaseOrdersHasError` are never merged — the two requests fail independently and the template must still render "vendor not found" regardless of PO-history state.
- **`aw-card` (shared/ui) powers the three spend-metric tiles** (Total Spend, Purchase Orders, Avg. PO Value) — follows `SalesPersonDetailComponent`'s existing stat-tile grid precedent (`sales-person-detail.html:109-146`); follow this same shape for future metric-tile UIs rather than hand-rolling `card bg-base-100` markup.
- **Preferred/Active badges reuse `StatusBadgeComponent`** with the exact map pattern `VendorListComponent`'s Status column already uses (`preferred`/`standard`), plus a second, independent map for `active`/`inactive` — two separate badge instances, not one shared map.
- **PO-history filter bar follows the List Component Invariants' URL-param-sync shape**: Reactive Forms, `router.navigate([], { queryParamsHandling: 'merge' })` on submit/reset, and a `route.queryParams` subscription as the sole driver of reloads — reusing the `parseFilterParams`/`restoreFiltersFromUrl`/`readFilters` triad from `VendorListComponent`, including the same out-of-range-value-resets-to-"All" handling for the `status` filter.
- **PO id column renders via `routerLink` inside an `awColumnDef` template cell**, not a row-click event — `DataTableComponent` has no row-click output; only `sortChange`/`pageChange`.
