...
describe('EmployeeStore', () => {
  ...
  describe('refresh', () => {
    it('replays the last load page request', () => {
      ...
      const firstReq = httpTesting.expectOne('https://api.test.com/v1/employees?pageNumber=2&pageSize=5');
      firstReq.flush({ ...mockSearchResult, pageNumber: 2, pageSize: 5 });

      store.refresh();
      const refreshReq = httpTesting.expectOne('https://api.test.com/v1/employees?pageNumber=2&pageSize=5');
    ...
    it('captures the latest list/query request and replays it', () => {
      store.loadPage({ pageNumber: 1, pageSize: 10 });
      const loadReq = httpTesting.expectOne('https://api.test.com/v1/employees?pageNumber=1&pageSize=10');
      loadReq.flush(mockSearchResult);

      store.search({ params: { pageNumber: 3, pageSize: 20 }, body: { firstName: 'Taylor' } });
      const searchReq = httpTesting.expectOne('https://api.test.com/v1/employees/search?pageNumber=3&pageSize=20');
      searchReq.flush({ ...mockSearchResult, pageNumber: 3, pageSize: 20 });

      store.refresh();
      const refreshReq = httpTesting.expectOne('https://api.test.com/v1/employees/search?pageNumber=3&pageSize=20');
      expect(refreshReq.request.method).toBe('POST');
      expect(refreshReq.request.body).toEqual({ firstName: 'Taylor' });
