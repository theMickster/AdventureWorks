...
public sealed class ReadScrapReasonQueryHandlerTests : UnitTestBase
{
    private readonly IMapper _mapper;
    private readonly Mock<IScrapReasonRepository> _mockRepository = new();
    private ReadScrapReasonQueryHandler _sut;

    public ReadScrapReasonQueryHandlerTests()
    {
        ...
        _sut = new ReadScrapReasonQueryHandler(_mapper, _mockRepository.Object);
    }
    ...
    public void Constructor_throws_correct_exceptions()
    {
        ...
        {
            _ = ((Action)(() => _sut = new ReadScrapReasonQueryHandler(
                    null!,
    ...
    public async Task Handle_returns_null_when_entity_not_found()
    {
        _mockRepository.Setup(x => x.GetByIdAsync(It.IsAny<int>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((ScrapReason)null!);

        var result = await _sut.Handle(new ReadScrapReasonQuery { Id = 9999 }, CancellationToken.None);
    ...
    public async Task Handle_returns_correctly_mapped_model()
    {
        ...
        var result = await _sut.Handle(new ReadScrapReasonQuery { Id = 1 }, cancellationToken);

        using (new AssertionScope())
        {
            result.Should().NotBeNull();
            result.Should().BeOfType<ScrapReasonModel>();
            result!.ScrapReasonId.Should().Be((short)1);
            result.Name.Should().Be("Brake assembly not as ordered");
