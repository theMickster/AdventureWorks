...
public sealed class StoreRepositorySearchTests : PersistenceUnitTestBase
{
    ...
    public async Task GetStoresAsync_succeeds_when_sorted_by_BusinessEntityIdAsync()
    {
        ...
        var (ascResults, ascTotalCount) = await _sut.GetStoresAsync(ascParams, cancellationToken: TestContext.Current.CancellationToken);
        var (descResults, descTotalCount) = await _sut.GetStoresAsync(descParams, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SearchStoresAsync_by_id_succeedsAsync(int storeId, string sortOrder)
    {
        ...
        var (ascResults, totalCount) = await _sut.SearchStoresAsync(queryParams, searchParams, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SearchStoresAsync_by_name_succeedsAsync(string storeName, string sortOrder, int firstStoreId, int totalCount)
    {
        ...
        var (ascResults, totalCountOutput) = await _sut.SearchStoresAsync(queryParams, searchParams, cancellationToken: TestContext.Current.CancellationToken);
        var firstRecord = ascResults.First();
