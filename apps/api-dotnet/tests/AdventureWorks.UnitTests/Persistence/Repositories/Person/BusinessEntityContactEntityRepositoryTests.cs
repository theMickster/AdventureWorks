...
public sealed class BusinessEntityContactEntityRepositoryTests : PersistenceUnitTestBase
{
    ...
    {
        const int id = 1112;
        var result = await _sut.GetContactsByIdAsync(id, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetContactsByStoreIdsAsync_succeedsAsync()
    {
        ...
        var results = await _sut.GetContactsByStoreIdsAsync(storeIds, cancellationToken: TestContext.Current.CancellationToken);
